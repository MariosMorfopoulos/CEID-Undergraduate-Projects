...
void RamerDouglasPeucker(const vector<Point> &pointList, double epsilon, vector<Point> &out)
{
	...
	if (dmax > epsilon) {
		...
		//{
		#pragma omp task shared(recResults1) firstprivate(epsilon,firstLine)
		RamerDouglasPeucker(firstLine, epsilon, recResults1);
		
		#pragma omp task shared(recResults2) firstprivate(epsilon,lastLine)
...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel shared(SimplifiedAllPolylines) firstprivate(polyline,epsilon,AllPolylines) private(simplified_polyline) num_threads(4) 
	{
