	...
	
{
	...
	PolyNum = AllPolylines.size();
	
	#pragma omp parallel default(shared) private(t_start,t_end, t_time,numOfThreads, chunk, polyline, simplified_polyline) \
		firstprivate(PolyNum, epsilon) num_threads(4)
	{
		t_start = omp_get_wtime();
		numOfThreads = omp_get_num_threads();	
		chunk = PolyNum/(numOfThreads*10000);
	#pragma omp for schedule(dynamic,chunk) private(polyline, simplified_polyline) nowait
	for (vector< vector<Point> >::const_iterator it1 = AllPolylines.begin(); it1 < AllPolylines.end(); it1++) {
		...
		printf("Time for thread %d = %13.6f sec\n", omp_get_thread_num() ,t_time);
